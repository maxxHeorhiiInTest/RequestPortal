<?php
/**
 * Admin: Kanban board of questions / suggestions.
 */
...
<div class="kanban" id="kanban"
     data-move-url="<?= e(rp_url('admin/feedback-move.php')) ?>"
    ...
    <?php foreach ($columns as $status => $cards): ?>
        <section class="kanban-col" data-status="<?= e($status) ?>">
            ...
            <p class="kanban-empty"<?= $cards ? ' hidden' : '' ?>><?= e(__('admin.feedback.board_empty')) ?></p>
            <?php foreach ($cards as $card): ?>
                <a class="kanban-card"
                   draggable="true"
                   href="<?= e(rp_url('admin/feedback-view.php?id=' . (int) $card['id'])) ?>"
                   data-id="<?= (int) $card['id'] ?>"
                   data-status="<?= e((string) $card['status']) ?>">
                    <span class="code-cell"><?= e((string) $card['public_code']) ?></span>
                    <span class="target"><?= e(mb_strimwidth((string) $card['message'], 0, 80, '…', 'UTF-8')) ?></span>
                    <span class="meta">
                        <?= e((string) $card['requester_name']) ?>
                        <?php
                        $unit = trim((string) ($card['faculty'] ?? '') . ' / ' . (string) ($card['department'] ?? ''), ' /');
                        if ($unit !== ''):
                        ?>
                            · <?= e($unit) ?>
                        <?php endif; ?>
                        · <?= e(rp_format_datetime((string) $card['created_at'])) ?>
                        <?php if ((int) $card['file_count'] > 0): ?>
                            · <?= (int) $card['file_count'] ?>
                        <?php endif; ?>
                    </span>
                </a>
