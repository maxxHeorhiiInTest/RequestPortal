<?php
/**
 * Admin: Kanban board of IT support tickets.
 */
...
$columns = [];
foreach (rp_it_statuses() as $status) {
    $columns[$status] = [];
}
...
<div class="kanban" id="kanban"
     data-move-url="<?= e(rp_url('admin/it-move.php')) ?>"
    ...
    <?php foreach ($columns as $status => $cards): ?>
        <section class="kanban-col" data-status="<?= e($status) ?>">
            ...
            <p class="kanban-empty"<?= $cards ? ' hidden' : '' ?>><?= e(__('admin.it.board_empty')) ?></p>
            <?php foreach ($cards as $card): ?>
                <a class="kanban-card"
                   draggable="true"
                   href="<?= e(rp_url('admin/it-view.php?id=' . (int) $card['id'])) ?>"
                   data-id="<?= (int) $card['id'] ?>"
                   data-status="<?= e((string) $card['status']) ?>">
                    <span class="code-cell"><?= e((string) $card['public_code']) ?></span>
                    <span class="target"><?= e(rp_it_category_label((string) $card['category'])) ?></span>
                    <span class="target"><?= e(mb_strimwidth((string) $card['description'], 0, 80, '…', 'UTF-8')) ?></span>
                    <span class="meta">
                        <?= e((string) $card['requester_name']) ?>
                        · <?= e((string) $card['building']) ?> / <?= e((string) $card['room']) ?>
                        · <?= e(rp_format_datetime((string) $card['created_at'])) ?>
                    </span>
                </a>
            <?php endforeach; ?>
        </section>
...
(function () {
    ...
    var dragging = null;
    var didDrag = false;
    function columnOf(el) { return el && el.closest ? el.closest('.kanban-col') : null; }
    ...
    board.addEventListener('dragend', function () {
        ...
        dragging = null;
        board.querySelectorAll('.kanban-col').forEach(function (col) { col.classList.remove('drag-over'); });
    });
    ...
    board.addEventListener('drop', function (event) {
        ...
        var card = dragging;
        var empty = col.querySelector('.kanban-empty');
        if (empty && empty.nextSibling) col.insertBefore(card, empty.nextSibling);
        ...
        fetch(moveUrl, {
            ...
            body: JSON.stringify({ csrf_token: csrf, id: Number(card.getAttribute('data-id')), status: newStatus })
        }).then(function (response) {
            return response.json().then(function (data) {
                return { ok: response.ok && data && data.ok };
            });
